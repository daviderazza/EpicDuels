    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("daccept")) {
            if (requests.containsKey(player.getUniqueId())) {
                player.sendMessage("§7Hai accettato la richiesta di duello di " + Bukkit.getPlayer(requests.get(player.getUniqueId())));
                Bukkit.getPlayer(requests.get(player.getUniqueId())).sendMessage("§7" + player.getName() + " ha accettato la tua richiesta di duello");
                Bukkit.getPlayer(requests.get(player.getUniqueId())).teleport(player);
                requests.remove(player.getUniqueId());
                setPlayersOnDuel(player);
                return true;
            }
            player.sendMessage("§cNon hai nessuna richiesta da accettare");
        }

        if (command.getName().equalsIgnoreCase("ddeny")) {
            if (requests.containsKey(player.getUniqueId())) {
                player.sendMessage("§7Hai rifiutato la richiesta di duello di " + Bukkit.getPlayer(requests.get(player.getUniqueId())));
                Bukkit.getPlayer(requests.get(player.getUniqueId())).sendMessage("§7" +  player.getName() + " ha rifiutato la tua richiesta di duello");
                requests.remove(player.getUniqueId());
                return true;
            }
            player.sendMessage("§cNon hai nessuna richiesta da rifiutare");
        }
        return true;
    }


    public void setPlayersOnDuel(Player player){
        playersOnDuel = true;
    }

    public Boolean getPlayersOnDuel(Player player){
        return playersOnDuel;
    }
}
